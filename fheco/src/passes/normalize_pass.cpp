...
void Normalizer::flatten_by_one_level(const ir::Term::Ptr &node)
{
  ...
  auto flatten_condition = [this](const ir::Term::Ptr &parent_node, const ir::Term::Ptr &node) {
  ...
  std::vector<ir::Term::Ptr> operands = node->get_operands();
  node->clear_operands();
...
void Normalizer::normalize_rotations(const ir::Term::Ptr &node)
{
  // we care only about rotations
  if (target_arithmetic_opcodes.find(node->get_opcode()) == target_arithmetic_opcodes.end())
    return;

  std::vector<ir::Term::Ptr> rotation_nodes;
  /*
  ...
  */
  auto rot_comp = [](const ir::Term::Ptr &lhs, const ir::Term::Ptr &rhs) {
...
void Normalizer::recover_binary_form(ir::Term::Ptr &node)
{
  if (node->is_operation_node() == false)
    return;
...
ir::Term::Ptr Normalizer::recover_binary_form_helper(
  const ir::Term::Ptr &origin_node, const std::vector<ir::Term::Ptr> &operands, size_t start_index, size_t end_index)
{
  ...
  ir::Term::Ptr lhs = recover_binary_form_helper(origin_node, operands, start_index, mid);
