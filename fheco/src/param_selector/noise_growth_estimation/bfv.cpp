...
int main()
{
  ...
  while (poly_mod <= max_poly_mod)
  {
    ...
    {
      cout << "\tplain_mod_size: " << plain_mod_size << endl;
      ...
      auto plain_mod_esimations_it = bfv_noise_experiments.find(plain_mod_size);
      if (plain_mod_esimations_it == bfv_noise_experiments.end())
        bfv_noise_experiments.insert({plain_mod_size, {{poly_mod, noise_estimates}}});
      else
        plain_mod_esimations_it->second.insert({poly_mod, noise_estimates});

      ++plain_mod_size;
    }
    poly_mod = poly_mod << 1;
  }
  serialize_bfv_noise_experiments(bfv_noise_experiments, "bfv_noise_experiments.txt");
  return 0;
}
