...
std::shared_ptr<ir::Term> subtract_scalars(const std::vector<std::shared_ptr<ir::Term>> &scalars, ir::Program *program)
{
  ...
  if (program->get_encryption_scheme() != fhecompiler::ckks)
  {
    std::int64_t to_int = subtraction;
    new_scalar_value = to_int;
  }
  else
  {
...
double get_constant_value_as_double(ir::ConstantValue const_value)
{
  ...
  if (auto v = std::get_if<std::int64_t>(&scalar_value))
...
void cast_int_vector_to_double(const std::vector<std::int64_t> &int_vector, std::vector<double> &double_vector)
{
...
void cast_int_vector_to_double(const std::vector<std::uint64_t> &int_vector, std::vector<double> &double_vector)
{
  if (double_vector.size() < int_vector.size())
    double_vector.resize(int_vector.size());
...
void cast_double_vector_to_int(const std::vector<double> &double_vector, std::vector<std::int64_t> &int_vector)
{
...
void get_constant_value_as_vector_of_double(ir::ConstantValue const_value, std::vector<double> &double_vector)
{
  ...
  if (auto v = std::get_if<std::vector<std::int64_t>>(&vector_value))
  ...
  const std::vector<std::shared_ptr<ir::Term>> &const_plains, ir::Program *program)
{
  ...
  for (size_t i = 0; i < const_plains.size(); i++)
  {
    ...
    if (
      ...
      const_plain->get_term_type() != ir::TermType::plaintext)
    {
...
std::shared_ptr<ir::Term> fold_ir_term(const std::shared_ptr<ir::Term> &term, ir::Program *program)
{
  ...
  if (folded_rhs->get_term_type() == ir::TermType::scalar && folded_lhs->get_term_type() == ir::TermType::scalar)
  {
  ...
  bool is_lhs_a_const_plain = (folded_lhs->get_term_type() == ir::TermType::plaintext) &&
                              (program->type_of(folded_lhs->get_label()) == ir::ConstantTableEntryType::constant);

  bool is_rhs_a_const_plain = (folded_rhs->get_term_type() == ir::TermType::plaintext) &&
