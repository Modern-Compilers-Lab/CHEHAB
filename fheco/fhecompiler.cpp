...
void compile(const std::string &output_filename)
{
  ...
  fheco_trs::TRS trs(program);
  {
    if (ENABLE_OPTIMIZATION)
    {
      for (size_t i = NB_TRS_CSE_PASS; i > 0; i--)
      {
        ...
        trs.apply_rewrite_rules_on_program();
        cse_pass.apply_cse2();
        std::cout << "pass# " << i << "\n";
      }
    }
...

  {
    translator::Translator tr(program);
    {
      ...
      tr.translate_program(translation_source_os, translation_header_os, 20);

      translation_source_os.close();
      translation_header_os.close();
    }
  }
