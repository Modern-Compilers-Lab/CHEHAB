...
void dot_product_baseline(size_t slot_count)
{
  Ciphertext c0("c0", -10, 10);
  Plaintext v1("v1", -10, 10);
  Ciphertext slot_wise_mul = c0 * v1;
  Ciphertext sum = reduce_add(slot_wise_mul);
  sum.set_output("result");
}
...
int main(int argc, char **argv)
{
  ...
  Compiler::compile(opt_func, ruleset, rewrite_heuristic, opt_header_os, opt_gen_name + ".hpp", opt_source_os, true);
