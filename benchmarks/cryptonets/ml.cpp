  ...
  const vector<size_t> &strides)
{
  ...
  vector<vector<vector<Ciphertext>>> output(
    n_rows_output,
    vector<vector<Ciphertext>>(n_cols_output, vector<Ciphertext>(n_channels_output, Ciphertext::encrypt({0}))));
  size_t row_offset = 0;
  ...
  const vector<vector<vector<Ciphertext>>> &input, const vector<size_t> &kernel_size, const vector<size_t> &strides)
{
  ...
  vector<vector<vector<Ciphertext>>> output(
    ...
    vector<vector<Ciphertext>>(n_cols_input + pad_cols, vector<Ciphertext>(n_channels, Ciphertext::encrypt({0}))));
