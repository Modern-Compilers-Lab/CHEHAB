  ...
  const vector<size_t> &strides)
{
  ...
  Plaintext pt(std::vector<int64_t>({0}));
  Ciphertext zero = Ciphertext::encrypt(pt);
  vector<vector<vector<Ciphertext>>> output(
    n_rows_output, vector<vector<Ciphertext>>(n_cols_output, vector<Ciphertext>(n_channels_output, zero)));
  size_t row_offset = 0;
  ...
  const vector<vector<vector<Ciphertext>>> &input, const vector<size_t> &kernel_size, const vector<size_t> &strides)
{
  ...
  Ciphertext zero = Ciphertext::encrypt(pt);
  vector<vector<vector<Ciphertext>>> output(
    n_rows_input + pad_rows, vector<vector<Ciphertext>>(n_cols_input + pad_cols, vector<Ciphertext>(n_channels, zero)));
...
vector<Ciphertext> dot(const vector<Ciphertext> &input, const vector<vector<Plaintext>> &w)
{
  ...
  Plaintext pt(std::vector<int64_t>({0}));
  Ciphertext zero = Ciphertext::encrypt(pt);
  vector<Ciphertext> output(w[0].size(), zero);
...
{
  vector<Ciphertext> output(input.size() * input[0].size() * input[0][0].size());
  size_t i_output = 0;
