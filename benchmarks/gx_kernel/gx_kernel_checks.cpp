...
void gx_kernel_baseline(size_t width)
{
  Ciphertext img("img", 0, 255);
  Ciphertext top_sum = -(img >> (width + 1)) + (img >> (width - 1));
  Ciphertext curr_sum = -2 * (img >> 1) + 2 * (img << 1);
  Ciphertext bottom_sum = -(img << (width - 1)) + (img << (width + 1));
  Ciphertext result = top_sum + curr_sum + bottom_sum;
  result.set_output("result");
}
