...
pub fn rules0(vector_width: usize) -> Vec<Rewrite<VecLang, ConstantFold>> {
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        ...
        //rw!("comm-factor-1"; "(* ?a0 (+ ?b0 ?c0))" => "(+ (* ?a0 ?b0) (* ?a0 ?c0))"),
    ];
    /************************************************************************************/
    // Vector rules
...
for iteration in 0..6 {
    ...
    let (cost, best) = rules::run(&best_expr, timeout, vector_width,rulesets_appplying_order[iteration%rulesets_appplying_order.len()]);
...
/***************************GX 16**************************************************************************/
/**********************************************************************************************************/
let mut rulesets_appplying_order  = vec![0];
    let mut best_cost = 0;
    let mut best_expr = prog;
    for iteration in 0..6 {
        //println!("Iteration: {}", iteration + 1);
    ...
    rulesets_appplying_order  = vec![2,1];
...
pub fn run(
    ...
    match selected_ruleset_order {
        0 => {rules = rules0(vector_width);},
        1 => {rules = vector_rules(vector_width);},
        2 => {rules = multiplication_rules(vector_width);},
        _ => println!("Ruleset correspoding to this order doesnt exist"),
    }
    let mut iteration_count = 0;
    // Start timing the e-graph building process
...
/************************************************************************************************************/
/**************************** BOX_BLUR 8 , 16 GX_kernel GY_Kernel 4 ,8 *************************************/
/***********************************************************************************************************/
    let mut rulesets_appplying_order  = vec![0,1];
    let mut best_cost = 0;
    let mut best_expr = prog;
...
pub fn run(
    ...
    match selected_ruleset_order {
        ...
        1 => {rules = vector_rules(vector_width);},
        2 => {rules = multiplication_rules(vector_width);},
        3 => {rules = addition_rules(vector_width);},
        _ => println!("Ruleset correspoding to this order doesnt exist"),
    }
    let mut iteration_count = 0;
    // Start timing the e-graph building process
