...
pub fn vec_with_op(op: &String, pre: &String, vector_width: usize) -> String {
    let joined = ids_with_prefix(pre, vector_width).join(" ");
    format!("({} {})", op, joined)
}

pub fn vec_with_var(pre: &String, vector_width: usize) -> String {
    vec_with_op(&"Vec".to_string(), pre, vector_width)
}
...
pub fn vec_map_op(op: &String, pre: &String, vector_width: usize) -> String {
    let mut ops: Vec<String> = Vec::with_capacity(vector_width);
    for i in 0..vector_width {
