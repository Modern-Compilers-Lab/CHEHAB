...
T& operate_assignement(T& lhs, const T& rhs, ir::TermType term_type)
{
  ...
  if(lhs_entry == std::nullopt) throw(" object node doesnt exist in dataflow when trying to overload assign operator for Ciphertext "); 
  ...
  if( program->type_of(lhs.get_label()) == ir::ConstantTableEntryType::output )
  {
    ...
    lhs.set_new_label();
    program->insert_new_entry_from_existing_with_delete(lhs.get_label(), old_label);
    auto new_assign_operation = program->insert_operation_node_in_dataflow(ir::OpCode::assign, {ct_copy_node_ptr}, lhs.get_label(), term_type);
  }

  else lhs.set_label(rhs.get_label());
...
inline void operate_in_constants_table(const std::string& label, const std::string& tag, fhecompiler::VarType var_type)
{
  if( tag.length() )
  {
    ...
    if( var_type == fhecompiler::VarType::input ) entry_type = ir::ConstantTableEntryType::input;
    else if( var_type == fhecompiler::VarType::output ) entry_type = ir::ConstantTableEntryType::output;
    else if( var_type == fhecompiler::VarType::constant) entry_type = ir::ConstantTableEntryType::constant;
    else entry_type = ir::ConstantTableEntryType::constant;
