...
namespace util
{
  ...
  void Quantifier::compute_he_depth_info()
  {
    ...
    {
      if (term->type() == ir::TermType::ciphertext && term->is_source())
      ...
      while (!dfs.empty())
      {
        ...
        if (
          top_term->op_code().type() == ir::OpCode::Type::mul || top_term->op_code().type() == ir::OpCode::Type::square)
          ++operands_xdepth;
        for (const auto operand : top_term->operands())
        {
          if (operand->type() == ir::TermType::ciphertext)
  ...
  void Quantifier::count_terms_classes()
  {
    ...
    for (const auto &term : func_->get_top_sorted_terms())
    {
      ...
      {
        if (term->type() == ir::TermType::ciphertext)
        {
          if (term->op_code().type() == ir::OpCode::Type::mul)
          {
            if (term->operands()[0] == term->operands()[1])
            {
              ...
              auto [it, inserted] = cc_mul_count_.emplace(
                CCOpInfo{
                  arg1_info.opposed_level_, min(arg1_info.size_, arg2_info.size_),
            ...
            {
              const auto &ctxt_arg_info = (term->operands()[0]->type() == ir::TermType::ciphertext)
              ...
              ++captured_terms_count_;
              auto [it, inserted] =
                cp_mul_count_.emplace(CAOpInfo{ctxt_arg_info.opposed_level_, ctxt_arg_info.size_}, 1);
              if (!inserted)
                ++it->second;
            }
          }
          ...
          else if (term->op_code().type() == ir::OpCode::Type::negate)
          {
            ...
            ++captured_terms_count_;
            auto [it, inserted] =
              he_add_sub_negate_count_.emplace(CAOpInfo{arg_info.opposed_level_, arg_info.size_}, 1);
            if (!inserted)
              ++it->second;
          }
          else
